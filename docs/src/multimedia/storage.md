...
`alia_proxy` 的所有多媒体持久化功能都依赖于其强大的本地存储系统。这个系统被设计为既简单又高效，确保了所有生成的媒体资源都能被安全地存储和访问。
...
- **异步写入**: 所有的文件写入操作都是通过 `aiofiles` 库以**异步**方式执行的。这意味着文件 I/O 操作不会阻塞 FastAPI 的主事件循环，保证了即使在处理大文件时，`alia_proxy` 依然能保持高并发和低延迟。
...
`alia_proxy` 的一个重要设计决策是**避免在数据库中存储大型二进制数据或 Base64 字符串**。
