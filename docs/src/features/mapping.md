...
除了简单的别名映射，`aiprox` 还支持将一个别名映射到**多个目标**，并提供了不同的流量分配策略。这为实现负载均衡、故障转移和成本优化提供了强大的工具。
...
当客户端请求 `"model": "smart-pool"` 时，`aiprox` 会根据指定的策略从 `targets` 列表中选择一个目标来处理请求。
...
- **关键字**: `round-robin`
- **行为**: 这是默认策略。`aiprox` 会按顺序将请求依次分配给 `targets` 列表中的每个目标。当到达列表末尾时，它会从头开始。
- **示例**:
...
- **关键字**: `random`
- **行为**: `aiprox` 会从 `targets` 列表中**随机选择一个**目标来处理每个请求。
- **适用场景**:
