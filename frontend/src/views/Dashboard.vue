  ...
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div v-for="(count, provider) in stats?.provider_counts" :key="provider" class="space-y-2">
            ...
            <div class="w-full bg-gray-100 dark:bg-gray-700 rounded-full h-2">
              <div class="bg-blue-600 h-2 rounded-full transition-all duration-500"
                :style="{ width: `${(count / (stats?.total_requests || 1)) * 100}%` }"></div>
            </div>
          </div>
          <div v-if="!stats?.provider_counts || Object.keys(stats.provider_counts).length === 0"
      ...
      <div class="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
        ...
        <div class="space-y-4">
          ...
          <div v-if="!stats?.model_counts || Object.keys(stats.model_counts).length === 0"
            class="text-center py-8 text-gray-400">
            暂无数据
          </div>
        </div>
      ...
      <div
        class="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm col-span-full lg:col-span-1">
        <h4 class="text-base font-semibold text-gray-900 dark:text-white mb-6">Token 消耗 TOP 5 模型</h4>
        <div class="space-y-4">
          <div v-for="(item, index) in stats?.top_models_by_tokens" :key="item.model" class="space-y-2">
            <div class="flex justify-between text-sm">
              <span class="text-gray-700 dark:text-gray-300 font-medium">{{ item.model }}</span>
              <span class="text-gray-500">{{ item.total_tokens }} Tokens</span>
            </div>
            <div class="w-full bg-gray-100 dark:bg-gray-700 rounded-full h-2">
          ...
          <div v-if="!stats?.top_models_by_tokens || stats?.top_models_by_tokens.length === 0"
            class="text-center py-8 text-gray-400">
            暂无数据
          </div>
        </div>
    ...
    <div
      ...
      <div class="p-6 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
        ...
        <button @click="fetchHealth" :disabled="loadingHealth"
      ...
      <div class="divide-y divide-gray-200 dark:divide-gray-700">
        ...
        <div v-if="!health?.providers || Object.keys(health.providers).length === 0"
