<template>
  <div class="space-y-6">
    ...
    <div v-if="selectedLog"
      ...
      <div
        class="bg-white dark:bg-gray-800 w-full max-w-4xl max-h-[90vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        <div class="flex-1 overflow-auto p-6 space-y-6">
          <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div class="p-4 bg-gray-50 dark:bg-gray-900/50 rounded-xl">
              ...
              <span :class="getStatusClass(selectedLog.status_code)"
                class="px-2 py-0.5 rounded text-[10px] font-bold uppercase">{{ selectedLog.status_code }}</span>
            </div>
          </div>
          ...
          <div class="space-y-4">
            <div class="space-y-2">
              ...
              <pre
                class="p-4 bg-gray-900 text-gray-300 rounded-xl text-xs overflow-auto max-h-60 whitespace-pre-wrap font-mono">{{ selectedLog.content?.prompt }}</pre>
            </div>
            <div class="space-y-2">
              <h4 class="text-sm font-bold text-gray-900 dark:text-white flex items-center gap-2">
              ...
              <pre
                class="p-4 bg-gray-900 text-gray-300 rounded-xl text-xs overflow-auto max-h-60 whitespace-pre-wrap font-mono">{{ selectedLog.content?.response }}</pre>
            </div>
            <div v-if="selectedLog.content?.error" class="space-y-2">
              ...
              <pre
                class="p-4 bg-red-900/20 text-red-400 border border-red-900/50 rounded-xl text-xs overflow-auto whitespace-pre-wrap font-mono">{{ selectedLog.content?.error }}</pre>
            </div>
            <div v-if="selectedLog.media && selectedLog.media.length > 0" class="space-y-2">
              ...
              <div v-for="m in selectedLog.media" :key="m.file_path"
                class="p-4 bg-gray-50 dark:bg-gray-900/50 rounded-xl flex items-center justify-between mb-2">
                <span class="text-xs font-mono truncate mr-4">{{ m.file_path }}</span>
                <a :href="`/api/media/${m.file_path.split('/').pop()}`" target="_blank"
                  class="text-xs bg-purple-600 hover:bg-purple-700 text-white px-3 py-1 rounded-lg transition-colors">查看媒体</a>
              </div>
            </div>
          </div>
        </div>
        <div class="p-6 border-t border-gray-200 dark:border-gray-700 flex justify-end gap-3">
          <button @click="onDeleteLog(selectedLog.id)"
            class="text-red-500 hover:text-red-600 text-sm font-medium mr-auto">删除记录</button>
          <button @click="selectedLog = null"
            class="bg-gray-900 dark:bg-white text-white dark:text-gray-900 px-6 py-2 rounded-xl text-sm font-bold transition-all hover:scale-105 active:scale-95">关闭</button>
        </div>
      </div>
    </div>
  </div>
